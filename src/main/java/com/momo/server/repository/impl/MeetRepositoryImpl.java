...
public class MeetRepositoryImpl implements MeetRepository {
    ...
    public void removeMeetByTitle(String title) {

        Query findMeetQuery = new Query(Criteria.where("title").is(title));
    ...
    public Meet findMeet(String meetId) {
        ...
        Meet meetEntity = mongoTemplate.findOne(Query.query(Criteria.where("meetId").is(meetId)),
            Meet.class);
        return meetEntity;
    }
    ...
    // Meet에 user 추가하는연산
    @Override
    public void addUser(Meet meetEntity, ArrayList<BigInteger> userList,
